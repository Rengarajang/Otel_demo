pipeline {
  ...
  agent any
   ...
   /** stage('Test') {
	...
	stage('deploy App') {
         steps {	    
	  sh 'sudo docker run -d --privileged --pid=host --network otel_demo_default -p 9095:8080 -e PROVIDER1_URL_BASE=http://172.31.6.50:8090 --name otel-api 785131266845.dkr.ecr.us-east-1.amazonaws.com/otel-demo /bin/bash start-tail.sh' 
	   }
	}
